...
namespace TogglSyncShared.Services;
...
public class CurrentIdentityProvider : ICurrentIdentityProvider
{
    ...
    public CurrentIdentityProvider(IHttpContextAccessor httpContextAccessor, IConfiguration configuration)
    {
        ...
        {
            ExternalId = httpContext.User.Claims.First(f => f.Type == ExpandingClaimTypes.AppId).Value;
        }
        else
        {
            ...
            Email = httpContext.User.Claims.First(f => f.Type == ClaimTypes.Name).Value;
            Username = httpContext.User.Claims.First(f => f.Type == ClaimTypes.Name).Value;
        }
    }
